...
SampleCoach::doSubstitute()
{
    ...
    {
    	//robottoCodeEditStart
        //doFirstSubstitute();
        //S_first_substituted = true;
    	S_first_substituted = doFirstSubstitute();
        //robottoCodeEditEnd
        return;
    }
...
*/
//robottoCodeEdit void to bool
bool
SampleCoach::doFirstSubstitute()
{
	//robottoCode
	return doFirstSubstituteRobotto();
...
//robottoCode
bool
SampleCoach::doFirstSubstituteRobotto() {
	// read opponents config file
	int aim,
		centerDefense,
		sideDefense,
		centerAttack,
	...
	std::string oppoTeamName = world().theirTeamName();

	char nameBuf [256];

	std::string nameStr;

	if (oppoTeamName.length() == 0) {
		return false;
	}

	std::string line;
	std::ifstream oppoConfFile("opponent.conf");

	if (oppoConfFile.is_open()) {
		while ( oppoConfFile.good() ) {
			std::getline (oppoConfFile, line);

			if (line.length() > 5) {
				std::sscanf(line.c_str(), "%s %d %d %d %d %d %d",
						  nameBuf, &aim, &centerDefense, &sideDefense, &centerAttack, &sideAttack, &throughPass);
				nameStr = nameBuf;
				if(nameStr == oppoTeamName ||
				   nameStr == "DEFAULT") {
					break;
				}
			}
	...
	std::vector<std::vector<double> > hetroWeights =
	  FuzzyHetroMatcher::getHetroWeights(aim,
										 centerDefense,
										 sideDefense,
										 centerAttack,
										 sideAttack,
	...
	for (int i = 0; i < 5; i ++) {
		...
		for (int j = 0; j < 3; j ++) {
			currentWeightSum += hetroWeights[i][j];
		}
		hetroWeights [i][3] = currentWeightSum;
	}

	std::vector<int> rolesOrder;

	rolesOrder.push_back(kCenterBackIndex);
	rolesOrder.push_back(kSideBackIndex);
	rolesOrder.push_back(kHalfIndex);
	rolesOrder.push_back(kCenterForwardIndex);
	rolesOrder.push_back(kSideForwardIndex);
	...
	for (int i = 0; i < 5; i ++) {
		for (int j = 0; j < 5; j ++) {
			if (hetroWeights[j][3] < hetroWeights[i][3]) {
				tempWeights = hetroWeights[j];
				hetroWeights[j] = hetroWeights[i];
				hetroWeights[i] = tempWeights;

				tempOrder = rolesOrder[j];
				rolesOrder[j] = rolesOrder[i];
				rolesOrder[i] = tempOrder;
			}
		}
	}
	...
	for (int i = 0; i < 5; i ++) {
		...
		switch (rolesOrder[i]) {
			case kCenterBackIndex: {
				currentPlayers.push_back(3);
				currentPlayers.push_back(4);
			}
			break;
			case kSideBackIndex: {
				currentPlayers.push_back(5);
				currentPlayers.push_back(2);
			}
			break;
			case kHalfIndex: {
				currentPlayers.push_back(6);
				currentPlayers.push_back(7);
				currentPlayers.push_back(8);
			}
			break;
			case kCenterForwardIndex: {
				currentPlayers.push_back(11);
			}
			break;
			case kSideForwardIndex: {
				currentPlayers.push_back(10);
				currentPlayers.push_back(9);
			}
			break;
			default:
			break;
		}
		int currPlayersCount = currentPlayers.size();

		for (int j = 0; j < currPlayersCount; j ++) {
			const GlobalPlayerObject * p = world().teammate(currentPlayers[j]);
			if ( ! p )
			{
				std::cerr << config().teamName() << " coach: "
						  << " teammate " << currentPlayers[j] << " does not exist."
						  << " skip first substitution." << std::endl;
				dlog.addText( Logger::TEAM,
							  __FILE__": teammate %d does not exist. skip first substitution.",
							  currentPlayers[j] );
			...
			int type = getBestHetroType(candidates, hetroWeights[j]);
			if ( type != Hetero_Unknown )
			{
				substituteTo(currentPlayers[j], type);
			}
		}
	}
...
//robottoCode
int
SampleCoach::getBestHetroType(PlayerTypePtrCont & candidates,
	...
	int typesCount = candidates.size();

	for (int i = 0; i < typesCount; i++) {
		currEval = candidates[i]->realSpeedMax                () * weights[kSpeedIndex]   +
				   candidates[i]->cyclesToReachMaxSpeed       () * weights[kSpeedIndex]   +
				   candidates[i]->staminaIncMax               () * weights[kStaminaIndex] +
				   candidates[i]->getDashPowerToKeepMaxSpeed  () * weights[kStaminaIndex] +
				   candidates[i]->getOneStepStaminaComsumption() * weights[kStaminaIndex] +
				   candidates[i]->kickableArea                () * weights[kKickIndex];
	...
	if (bestEvalIndex != -1) {
		int id = candidates[bestEvalIndex]->id();
		candidates.erase(candidates.begin() + bestEvalIndex);
		return id;
	}
