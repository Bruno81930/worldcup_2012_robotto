...
//robottoCode
CooperativeAction Bhv_ChainAction::filterFirstAction(PlayerAgent * agent,
		                                CooperativeAction* firstAction) {
	if (firstAction->category() == CooperativeAction::Dribble) {
		return *firstAction;
	}

	if (firstAction->category() == CooperativeAction::Move) {
		return *firstAction;
	}
  ...
  /*
	if (wm.self().pos().x > 47.5 &&
		abs(wm.self().pos().y) > 8.5 &&
		abs(wm.self().pos().y) < 12 &&
		wm.self().unum() != 11) {
		firstAction->setCategory(CooperativeAction::Pass);
  ...
  */
	...
	if (wm.self().pos().x > 40 &&
		abs(wm.self().pos().y) < 7 &&
		firstAction->category() != CooperativeAction::Shoot) {
		firstAction->setCategory(CooperativeAction::Shoot);
...
Bhv_ChainAction::execute( PlayerAgent * agent )
{
    ...
    /*
    const CooperativeAction first_action =
      this->filterFirstAction(
        agent, (CooperativeAction*) &M_chain_graph.getFirstAction() );
    */
